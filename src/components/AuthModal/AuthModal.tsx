...
export const KeyAuthModal = () => {
  ...
  // @ts-ignore
  const dispalyMnemonic = (mnemonic: string | '') =>
    (userInput.current.value = mnemonic);
  ...
  const modalButtons = [
    <div>
      {isUserAuthenticated ? (
        ...
        <div>
          <Button key="generate" onClick={handleGenerate}>
            Generate
          </Button>
          <Button key="reset" onClick={handleReset}>
            Reset
          </Button>
          <Button key="proceed" onClick={handleProceed} disabled={!isKeyValidated}>
            Proceed
          </Button>
        </div>
      )}
  ...
  return (
    <div className="wallet-modal__top-level">
      <Modal
        title={isUserAuthenticated ? 'Logout and Clear' : 'Getting Started'}
      ...
      >
        {!isUserAuthenticated ? (
          <div className="key-setup__container">
            ...
            <div className="key-setup__subtitle">
          ...
          <div className="delete-storage__body">
            ...
            <p className="delete-storage__info">
              Are you sure you want to logout? Doing this will delete your keys from the
              browser and log you out
            </p>
          </div>
        )}
      </Modal>
      {keyModalVisible && (
        <UserKeyModal
          ...
          generatedKeys={localKey}
          toExecute={setIsUserAuthenticated}
        />
      )}
    </div>
  );
