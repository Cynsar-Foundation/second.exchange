...
import { useEarthStarContext } from "../service/earthstar";
...
const Home: NextPage = () => {
  ...
  const earthStarService = useEarthStarContext();
  ...
  useEffect(() => {
    if (!hasFetchedUsers && (setupServices.isNDKInitialized() || ndk)) {
      feedService.getUserDetails().then(() => {
        ...
        earthStarService.generateShares("sara");
      });
    }
    ...
    if (posts.length === 0) {
      return (
        ...
        >
          ...
          <Text>Maybe start following a list of user.</Text>
          {otherUsersTofollow && (
            <UserProfiles profileData={otherUsersTofollow} onFollow={"c"} />
          )}
        </Flex>
      );
    }
